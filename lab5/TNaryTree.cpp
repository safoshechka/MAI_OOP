...
template <class T>
int TNaryTree<T>::GetNodesOnLevel(std::shared_ptr<TTreeItem<T>> item, int level) {    //Функция показывающаю, сколько элементов на определенном уровне
...
template <class T>
void TNaryTree<T>::Pop(std::shared_ptr<TTreeItem<T>> item, size_t value) // Удаление элемента из дерева
{
    if (item->GetSon()) {
...
template <class A>
std::ostream& operator<<(std::ostream &os, const TNaryTree<A> &tree) { //Оператор вывода дерева
